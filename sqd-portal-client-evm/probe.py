...
from sqd_portal_client_evm.client import (
    get_multiple_data, get_multiple_data_async,
    chain_queries, chain_queries_async,
    combine_query_results, filter_combined_results,
    QueryChain
)
...
def example_basic_chaining():
    ...
    queries = [
        (Dataset.ETHEREUM, Query.transactions(from_address=VITALIK_ETH, from_block=17000000, to_block=17000010)),
        (Dataset.ETHEREUM, Query.logs_from_contract(USDC_ETH, from_block=17000000, to_block=17000010)),
        (Dataset.ETHEREUM, Query.simple_block_range(from_block=17000000, to_block=17000010))
    ]   
    ...
    for i, result in enumerate(results):
        print(f"Query {i + 1} returned {len(result) if isinstance(result, list) else 1} items")
...
async def example_async_chaining():
    ...
    queries = [
        (Dataset.ETHEREUM, Query.transactions(from_address=VITALIK_ETH, from_block=17000000, to_block=17000010)),
        (Dataset.ETHEREUM, Query.logs_from_contract(USDC_ETH, from_block=17000000, to_block=17000010)),
        (Dataset.ETHEREUM, Query.erc20_transfers(USDC_ETH, from_block=17000000, to_block=17000010))
    ]
...
def example_same_dataset_chaining():
    ...
    queries = [
        Query.transactions(from_address=VITALIK_ETH, from_block=17000000, to_block=17000010),
...
async def example_fluent_chaining():
    ...
    results = (chain
               .add(Query.transactions(from_address=VITALIK_ETH, from_block=17000000, to_block=17000010))
...
def example_result_combination():
    ...
    print(f"Concatenated: {len(concatenated) if isinstance(concatenated, list) else 1} items")
    ...
    print(f"Filtered results: {len(filtered) if isinstance(filtered, list) else 1} items")
...
if __name__ == "__main__":
    ...
    try:
        ...
        print("Note: These examples require actual SQD portal access to run successfully.")
